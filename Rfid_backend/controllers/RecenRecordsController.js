...
const RecentRecordsoOnRfidAdmin = asyncHandler(async (req, res) => {
    const { rfid } = req.body;
    console.log(rfid);

    // const document = await HardwareRfidSwipe.find({ rfid }, { geoLocation: 0, Ip: 0 }).lean().exec();
    const document = await HardwareRfidSwipe.find({}, { rfid: 0, geoLocation: 0, Ip: 0 })
        .sort({ currentTime: -1 }) // Sorting by currentTime in descending order
        .lean()
        .exec();

    // console.log("Document 1 : ", document);

    const document2 = await EntryGateModel.find({}, { rfid: 0, geoLocation: 0, Ip: 0 })
    .sort({ currentTime: -1 }) // Sorting by currentTime in descending order
    .lean()
    .exec();
    ...
    const library = await LibraryModel.find({}, { geoLocation: 0, Ip: 0 })
        .sort({ currentTime: -1 }) // 1 for ascending order, -1 for descending order
        .lean()
        .exec();
    ...
    const Auditorium = await AuditoriumModel.find({}, { geoLocation: 0, Ip: 0 })
        .sort({ currentTime: -1 }) // 1 for ascending order, -1 for descending order
        .lean()
        .exec();
    // Combine both document1 and document2 arrays into a single array
